...
#################################### VOLLEY ##################################################
    ...
    def player1(self):     
        #player equipe 1
        return self.state.player_state(self.id_team, self.id_player).position<(GAME_WIDTH/2)
    
    @property
    def player2(self): #player equipe 2
        return self.state.player_state(self.id_team, self.id_player).position>(GAME WIDTH/2)
    ...
    @property
    def camp_adv(self):         ########"" dimension de chque camp de chaque equipe
        if(self.id_team==2):
    ...
    @property
    def passe_a_lui_meme(self):  # se fait la passe a lui meme
    ...
    def fait_la_passe_loin(self): # fait la passe le plus loin possible de l'adversaire
       ...
       #faut que le rayon ou je tire  ne depasse pas la limite
       
       #si le coequipier et a la premiere moitié du terrain: faire un shoot vers la 2eme moitié
        if(self.id_team==1):
            if(self.player2 < GAME_WEIGHT/3 ):
                    rayon= GAME_WIDTH-25
                    return self.player2-rayon
            else:
                rayon=GAME_WIDTH/2 +20
                return self.player2-rayon
            
          moitie= GAME_WEIGHT/2 # moitié du terrain
          if(self.id_team==2):
            if(self.player1 < GAME_WEIGHT/2 and self.player1 > moitie/2):
                    rayon= 25
                    return self.player1-rayon
            else:
                rayon=moite-20
                return self.player1-rayon
    ...
    @property   
    def court_vers_balle_anticipe(self): # court vers la balle des que la ball entre dans le terrain
        
        if(self.id_team==1):
            if(self.ball< GAME_WIDTH/2):
                return (self.ball+(self.ball_vitesse * (0.5*self.ball.distance(self.player1))))-self.player1
        else:
            if(self.ball>GAME_WIDTH/2):
                return (self.ball+(self.ball_vitesse * (0.5*self.ball.distance(self.player12)))-self.player2
